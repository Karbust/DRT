...
export default function RegistarUtilizador(props) {
    const classes = useStyles()

    const { tipoUser, tipoUserString, linkPagina } = props
    ...
    const onFormikSubmit = (values, formikActions) => {
        ...
        Object.keys(remaining_values).forEach((key) => {
            formData.append(key, remaining_values[key])
        })
                ...
                return (
                    <Box ml={10} mr={10}>
                        <Typography variant="h6" paragraph>
                            ...
                            {(() => {
                                if(tipoUser === 7){
                                    return ' e comprovativo_morada'
                                } else if (tipoUser === 5) {
                                    return ' e carta_conducao'
                                }
                            })()}
                        </Typography>
                        <FormDocumentos />
    ...
    return (
        <>
            <PaginasHeader
                ...
                severity={severity}
                message={message}
                titulo={`Registar ${tipoUserString}`}
                url={`/Dashboard/${linkPagina}`}
            />
            <Box mb={2}>
                <Stepper
...
RegistarUtilizador.propTypes = {
    tipoUser: PropTypes.number.isRequired,
    tipoUserString: PropTypes.string.isRequired,
